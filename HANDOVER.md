...
| **`Level change: Switch`** (third choice beside Restore/Manual, DEFAULT OFF, `cfg.autoSwitch` + `cfg.lastAppliedSet`): when the level crosses into a different BAND, the last-applied set is re-applied as the build for the new band. Silent when the live set already matches (a band change must not cost the 60s cast lock for nothing); suppresses the level-down report, which it supersedes. Fires 3s after the band moves (sync transitions bounce the structs) | Driven end-to-end against a stub client (`smoke: the level-change Switch rule`), **field round owed — this is the one that sends packets on its own.** |
