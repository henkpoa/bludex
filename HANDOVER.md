...
| Live set-point budget in the header | **FIELD-CONFIRMED, regression RESOLVED** — stuck on "reading..." early morning (survived zoning/job changes), then came back (`44 / 79` in the 09:xx screenshot) consistent with the blusets-documented quirk: private servers leave job structs stale until the native equip/Set Spells menus touch them. **Now self-healing (untested):** while "reading..." shows, `blu.nudgePoints()` injects the C2S 0x061 player-info request (the same ask the native menus fire; LSB answers with the job-extra battery) — ≤3 tries, 10s apart, re-armed by a good read. Manual: `/bludex refresh`. `/bludex debug` prints sig status + ungated raw reads. |
